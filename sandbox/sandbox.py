...
class GeoMapModule:
    ...
    def __init__(self, geo_model, grid: Grid, geol_map: Plot, work_directory=None):
        ...
        if self.geo_model.grid.topography.values.shape[0] is 0:
            self.geo_model.set_topography('random')
    ...
    def compute_model(self, kinect_array):
        ...
        self.geo_model.grid.topography.values = self.kinect_grid.depth_grid
        self.geo_model.grid.update_grid_values()
        self.geo_model.rescaling.set_rescaled_grid()

        # TODO IMP!!! This has to be only called the first time
        self.geo_model.update_to_interpolator()


        gp.compute_model(self.geo_model, compute_mesh=False, sort_surfaces=False)
        # lith_block = sol[0][0, :self.kinect_grid.depth_grid.shape[0]]
        # block = lith_block.reshape((self.kinect_grid.scale.output_res[1],
        #                             self.kinect_grid.scale.output_res[0]))
        # faults = sol[-1][:, :self.kinect_grid.depth_grid.shape[0]].astype('float')
        lith_block = self.geo_model.solutions.geological_map
        block = lith_block.reshape((self.kinect_grid.scale.output_res[1],
                                    self.kinect_grid.scale.output_res[0]))
        faults = numpy.empty((0, 0))#self.geo_model.solutions.[:, :self.kinect_grid.depth_grid.shape[0]].astype('float')
        return block, faults
