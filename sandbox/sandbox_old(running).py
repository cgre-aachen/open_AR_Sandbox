...
class KinectV2:
    ...
    def get_frame(self):
        ...
        print(depth_flattened.shape,self.depth.shape)
        self.depth = numpy.fliplr(self.depth)
        return self.depth
    ...
    def get_color(self):
        ...
        Returns:
               2D Array of the shape(424, 512) containing the color information of the latest frame

        """
        self.color = self.kinect.get_last_color_frame()
        #self.color = color_flattened.reshape((424, 512)) #reshape the array to 2D with native resolution of the kinectV2
...
class Projector:
    ...
    def show(self, input=None, legend_filename=None, profile_filename=None,
        ...
        for retry in range(100):#Windows sometimes  randomly throws an permission error here. With this i hope to catch it
            try:
              ...
              #  print("windows permission error happened, trying again...")
                pass
...
class GeoMapModule:
    ...
    def __init__(self, geo_model, grid: Grid, geol_map: Plot, work_directory=None):
        ...
        self.fault_line = self.create_fault_line(0, self.geo_model.geo_data_res.n_faults+0.500)
